...
namespace MoreMountains.Feedbacks
{
	...
	[FeedbackHelp("此反馈可让你以弹簧效果为目标对象的位置制作随时间变化的动画。")]
	public class MMF_PositionSpring : MMF_Feedback
	{
		...
		public override string RequiresSetupText { get { return "此反馈必须先设置an AnimatePositionTarget才能正常工作。你可以在下方进行设置。"; } }
		...
		[Tooltip("今年春天选择的模式。 MoveTo 会将目标移动到指定位置（在最小和最大之间随机）。 MoveToAdditive 会将指定位置（在最小值和最大值之间随机）添加到目标的当前位置。碰撞将以指定的功率（在最小和最大之间随机）碰撞目标的位置")]
		...
		[Tooltip("可用于确定目标位置的可选变换。如果设置了此项，则 MoveToPositionMin 和 MoveToPositionMax 值将被用作相加模式下此变换位置的偏移量")]
		[MMFEnumCondition("Mode", (int)Modes.MoveTo, (int)Modes.MoveToAdditive)]
