...
namespace MoreMountains.Feedbacks
{
	...
	[FeedbackHelp("此反馈可让你以弹簧效果为目标对象的缩放制作随时间变化的动画。")]
	public class MMF_ScaleSpring : MMF_Feedback
	{
		...
		[Tooltip("今年春天选择的模式。 MoveTo 会将目标移动到指定的比例（在最小和最大之间随机）。 MoveToAdditive 会将指定的比例（在最小值和最大值之间随机）添加到目标的当前比例。凹凸将以指定的功率（在最小和最大之间随机）改变目标的比例")]
