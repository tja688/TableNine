...
namespace MoreMountains.Feedbacks
{
	...
	[FeedbackHelp("此反馈可让你 emit a PositionShake event. This will be caught by MMPositionShakers (on the specified 通道)." +
	              " Position 抖动器s, as the name suggests, are used to shake 位置of a transform, along a direction, with optional noise and other fine control options.")]
	public class MMF_PositionShake : MMF_Feedback
	{
		...
		[Tooltip("要直接指定的抖动器，可选；设置后会忽略其通道限制")]
		...
		[Tooltip("摇动期间变换将移动到的距其初始位置的最大距离")]
		...
		[Tooltip("若开启此项，将不会直接使用 ShakeMainDirection 作为抖动方向，而是在 ShakeMainDirection 与 ShakeAltDirection 之间随机生成一个 Vector3")]
		public bool RandomizeDirection = false;
		/// when in RandomizeDirection mode, a vector against which to randomize the main direction
		[Tooltip("在 RandomizeDirection 模式下，用于随机化主方向的参考向量")]
		...
		[Tooltip("若开启此项，每次抖动时都会随机生成一个新方向")]
		...
		[Tooltip("启用方向噪声时，噪声强度会在此值与 DirectionalNoiseStrengthMax 之间随机")]
		...
		[Tooltip("启用方向噪声时，噪声强度会在此值与 DirectionalNoiseStrengthMin 之间随机")]
		[MMFCondition("AddDirectionalNoise", true)]
