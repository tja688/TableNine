...
本项目的核心不是传统卡牌“手牌/费用/回合”框架，而是一个**3×3 九宫格状态机 + 双卡组补牌 + 位置触发词条 + 帮助卡/遗物/技能效果流水线**。因此代码主干应以“规则状态”和“规则动作”为中心：
...
| `isPermanentRemoveOnUse` | 是否永久移除帮助卡。 |
...
关键规则：帮助卡每张有唯一 uid；节点开始保存快照；节点结束按快照恢复临时移除，再追加新获得卡；永久移除不恢复。
...
- `ClearReady` 不是自动进入下一节点；玩家还能拾取/使用帮助卡。
- `RoomChoosing` 点击房间后才结算未使用帮助卡金币并进入奖励/商店链路。
...
ValidateCombat
 ...
 -> DetermineInitiative
 -> FirstHit
 -> Thorn/side true-damage phase if applicable
 -> DeathCheck
 -> CounterHit if defender alive
 -> Damage prevention/reduction/reflect/death prevention
...
public sealed class DamageContext
{
    ...
    public DamageType type; // Combat, HelpCard, TrueDamage, Reflect, PoisonLike
...
- 普通战斗伤害 = 攻击 - 防御，最低 0。
- 先攻：只有一方有先攻时先攻方先；双方都有或双方都没有时玩家先。
- 致死者不反击。
- 庇佑只免疫一次伤害；多段只免第一段。
- 凤凰羽毛属于致命伤处理阶段，优先级高于死亡事件。
...
| `ConsumeHelpCardAtom` | 当前节点临时移除或永久移除。 |
...
| `ConsumeHelpCardCommand(cardUid, removeMode)` | 当前节点临时移除或永久移除。 |
...
| Query | 返回 | 用途 |
|---|---|---|
| `GetEffectivePlayerStatsQuery` | `EffectiveStats` | UI 显示、战斗结算。 |
...
CheckClearConditionCommand
 ...
 -> FlowPhase = ClearReady
 -> Send LevelClearReadyEvent
 -> UI 显示两个房间按钮
```

重要：`ClearReady` 状态下仍允许玩家拾取九宫格帮助卡、使用道具牌格帮助卡；只是不再可能战斗。点击房间按钮后才计算未使用帮助卡金币奖励并进入后续奖励/商店流程。

### 10.7 帮助卡恢复与金币结算

点击房间后：

1. 统计本节点未使用且回到卡组的帮助卡，每张 +10 金币。
2. 将帮助卡组从节点快照恢复。
3. 永久移除的帮助卡不恢复。
4. 节点中新获得的帮助卡在恢复后追加。
5. 清空本节点恶魔卡组与战斗牌堆。
...
StartCombatCommand(monsterUid)
 ...
 -> ApplyDamage(player -> monster)
 -> DeathCheck(monster)
 -> if monster alive: ApplyDamage(monster -> player)
 -> DeathCheck(player)
...
| 帮助卡 | 实现方式 |
|---|---|
| 恢复药水 | `HealAtom(10)` + `ConsumeHelpCardAtom(Permanent)`。 |
...
| 刺皮 | `OnAfterDamageReceived`，对攻击者造成其攻击力伤害。 |
...
- 已上传策划文档：`第四版设计.md`、`互动规则.md`、`帮助卡设计.md`、`怪物卡设计.md`、`技能.md`、`每关流程.md`、`遗物.md`、`职业.md`。
