...
4. 需要一个明确的屏幕回手区来区分回手与打出，并且卡牌没完全拖出该区域时仍允许反悔回手。
...
| 拖拽起手 | 长按或轻微位移超阈值都可起拖 |
| 回收区判定 | 统一用屏幕坐标检测 `mRecycleZoneViewportRect` |
| 区内回手 | 鼠标松手点在回手区内，或拖拽卡牌仍与回手区重叠 → 原卡缓动回当前手牌槽位 |
| 区外打出 | 鼠标与卡牌都已离开回手区后松手 → 直接销毁该卡并移出 Dock |
...
这轮把“回手”和“打出”继续分离，但修正成真正符合手感的屏幕区判定：

1. 回手区仍由 `mRecycleZoneViewportRect` 定义，并在 Scene 视图里可视化编辑。
2. 拖拽松手时，统一使用**同一套屏幕坐标**做检测，不再混用 GUI 坐标与 Screen 坐标。
3. 若满足任一条件，则视为回手：
   - 鼠标松手点落在回手区内
   - 被拖卡牌的屏幕包围盒仍与回手区重叠（也就是“还没完全拖出去”）
4. 只有当鼠标与卡牌都已经离开回手区后松手，才视为真正打出：
   - 直接销毁该卡 wrapper
   - 从 `mCards` 列表移除
   - 重新计算剩余卡牌排序与槽位
...
| `Recycle Zone` | 由 `mRecycleZoneViewportRect` 定义，Scene 视图可视化辅助调整 |
...
5. **回手区判定必须使用同一坐标空间**  
   之前把 viewport 换出来的屏幕矩形，和翻转过 Y 的 GUI 鼠标坐标混用，导致视觉上在区内，运行时却判不到，体感就是“一拖就打出”。  
...
5. 轻拖一张卡但不要把整张卡完全拖出右上回手区，松手后确认仍会回手，并准确贴回槽位
6. 将鼠标和整张卡都拖离回手区后松手，确认该卡被打出销毁，剩余卡牌自动均匀补位
