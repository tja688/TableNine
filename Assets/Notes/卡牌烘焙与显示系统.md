...
| Prefab 模板解析（Inspector 未拖引用时回退） | `Assets/Scripts/Game/CardBaking/BakedCardPrefabRefs.cs` |
...
| 消费方 | 行为 |
|--------|------|
| `GameplayWorldPresenter` | 实例化 `Card.prefab` 于 `NineGrid Main CardSlots`；事件驱动 `ComposeSet` + `Bind`；**仅同步领域层已落位卡牌**；物品槽高度 × 0.85 |
| `NineGridCardMoveDemo` | **发射器侧**牌堆 + 按 `1` 发外圈 8 格（纯表现层 Demo，不调用 `DealOpeningCardsCommand`）；详见 `NineGridCardMoveDemo九宫格外圈移动效果.md` |
...
### 8.1 Bootstrap 场景分工（玩家卡 vs 外圈发牌）

`TableNineBootstrap` 当前稳定链路把**中央玩家卡**与**外圈铺卡**拆开，避免 `GameplayWorldPresenter` 与发射器 Demo 叠两层卡：

```mermaid
flowchart TB
  subgraph bootstrap [Play 启动]
    GWP["GameplayWorldPresenter\nmBootstrapPlayerCardOnly"]
    SNR["StartNewRunCommand\nskipOpeningDeal: true"]
    GWP --> SNR
    SNR --> Player["领域层：仅格 5 玩家卡"]
    SNR -.->|跳过| DOC["DealOpeningCardsCommand\n（不外圈预铺）"]
    Player --> BCV5["BoardCardView5\nPlayerCard 模板烘焙"]
  end
  subgraph launcher [发射器 Demo]
    NG["NineGridCardMoveDemo\nCardDeckSlot 牌堆"]
    Key1["按 1"]
    NG --> Key1
    Key1 --> Outer["外圈 8 格视觉发牌\nNineGridCard_* 实例"]
  end
  BCV5 --> Hide["隐藏场景占位\nCardSlot5ForPlayer/PlayerCard"]
```

| 区域 | 负责方 | 数据来源 | 显示载体 |
|------|--------|----------|----------|
| 格 5 中央玩家卡 | `GameplayWorldPresenter` | `CardViewData` + `BakedCardFaceTemplate.PlayerCard` | `BoardCardView5`（`Card.prefab`） |
| 外圈 8 格 | `NineGridCardMoveDemo` | `CardDefinition` 池（Demo 牌堆） | `NineGridCard_*`（`Card.prefab`） |
| 正式开局外圈铺牌 | `DealOpeningCardsCommand` | 领域层牌堆 | **Bootstrap 演示期跳过** |

**关键约定：**

- 不要再在 `GameplayWorldPresenter` 里自动跑完整 `StartNewRunCommand()`（会触发 `DealOpeningCardsCommand` 把外圈铺满）。
- 演示期用 `StartNewRunCommand(skipOpeningDeal: true)`：只初始化 run、恶魔牌堆与格 5 玩家，外圈留空给发射器。
- `GameplayWorldPresenter` 只根据 `IBoardModel` 刷新 `BoardCardView*`；外圈无领域卡时对应 View 保持 `Hide()`。
- 场景占位 `CardSlot5ForPlayer/PlayerCard` 在 `BoardCardView5` 显示时自动隐藏，避免与烘焙单图叠影。

### 8.2 `GameplayWorldPresenter` Inspector

| 字段 | 默认 | 说明 |
|------|------|------|
| `mBootstrapPlayerCardOnly` | `true` | Play 时发送 `StartNewRunCommand(skipOpeningDeal: true)` |
| `mBoardRootName` | `NineGrid Main CardSlots` | 棋盘槽父节点（**不是** `NineGrid CardSlots`） |
| `mItemRootName` | `Item CardSlots` | 物品槽根；场景里可为 inactive，用 `FindSceneRoot` 解析 |
| `mCardFaceTemplate` / `mPlayerCardTemplate` | 可空 | 未拖引用时由 `BakedCardPrefabRefs` 在 Editor 下按路径加载 |
| `mScenePlayerCardPlaceholder` | 可空 | 默认解析 `CardSlot5ForPlayer/PlayerCard` |

### 8.3 玩家卡烘焙输入

`CardType.Player` → `BakedCardRenderDataFactory.CreatePlayerRuntimeData`：

- 模板：`PlayerCard.prefab`
- 数值：运行态 HP / 攻 / 甲
- 主图标：`CharacterDefinition.Image`
- 角标：玩家技能 + 未消耗遗物图标

测试：`TableNineCardBakingEditModeTests.PlayerRuntimeRenderData_Uses_PlayerCard_Template_And_Composes_Face`
...
### 11.1 `TableNineBootstrap` 快速验证（当前主场景）

1. 打开 `Assets/Scenes/TableNineBootstrap.unity` → Play
2. **中央格 5**：应出现烘焙后的玩家卡（`BoardCardView5`），场景占位 `PlayerCard` 被隐藏
3. **外圈 8 格**：开局为空槽，**不应**预铺怪物/帮助卡
4. **发射器**：`CardDeckSlot` 牌堆显示顶牌；按 **`1`** 连续发往外圈
5. 悬停外圈已发卡牌 / 顶牌 → `DescriptionText` 多行描述；外圈卡与中央玩家卡同步轻微放大

关闭仅引导玩家卡：在 `GameplayBootstrap` → `GameplayWorldPresenter` 取消 `Bootstrap Player Card Only`。
...
- `PlayerRuntimeRenderData_Uses_PlayerCard_Template_And_Composes_Face`
...
| 项 | 状态 | 说明 |
|----|------|------|
| Bootstrap 外圈与发射器双轨 | ✅ 已稳定 | 玩家卡走 `GameplayWorldPresenter` + `skipOpeningDeal`；外圈走 `NineGridCardMoveDemo` |
| 演示层外圈卡与领域层未同步 | ⚠️ 预期 | `NineGridCardMoveDemo` 发牌暂不写 `IBoardModel`；正式战斗需再接 Command |
...
| 2026-06-15 | Bootstrap 玩家卡正式落位：`PlayerCard.prefab` 烘焙 + `BoardCardView5`；`skipOpeningDeal` 取消外圈预铺；`BakedCardPrefabRefs` / `FindSceneRoot`；与发射器发牌分工写入 §8.1 |
