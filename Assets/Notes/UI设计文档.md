...
| `DescriptionPanel` | Web 有一个怪物技能详情面板，鼠标悬停怪物时显示怪物名、词条说明、动态 buff，例如战舞、皇室、庇佑、决斗。                                              | 你图里有两个 `DescriptionPanel`，建议其中一个明确做 `MonsterSkillTooltipPanel`，另一个做通用卡牌/遗物描述。至少需要：标题、词条列表、效果描述、动态状态行。                                                                     |
